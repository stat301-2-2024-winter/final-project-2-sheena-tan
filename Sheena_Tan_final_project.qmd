...
-   Support vector machine (`kernlab` engine)
    -   Degree of interaction was explored over $[1,3]$ with 3 levels
    -   Cost was explored over $[-10,5]$ with 5 levels (on log-2 scale)
...
The logistic model and support vector machine models used the preprocessing stored in `spotify_recipe_lm`. Variables used to define the target variable and identifier variables (`skip_1`, `skip_2`, `skip_3`, `not_skipped`, `session_id`, `track_id`, and variables with zero variance) were removed, nominal variables were dummy coded, and all numerical variables were standardized.
...
The models were compared using accuracy to determine the final model. Tuning parameters were compared for the support vector machine, k-nearest neighbors, random forest, and boosted tree models. The best performing parameters were:
...
The support vector machine model, k-nearest neighbors model, random forest model, and boosted tree model were fitted. The trained models were compared using accuracy to determine the final model. A table containing all of the models' assessment metrics (accuracy) can be seen below.
...
| svm      | 0.XXXXXXX | X.XXXXXXe-4 |
| knn      | 0.8283687 | 3.988367e-4 |

The support vector machine model had an accuracy of approximately **XX.XX%**; the k-nearest neighbors model had an accuracy of approximately **82.84%**; the random forest model had an accuracy of approximately **XX.XX%**; and the boosted tree model had an accuracy of approximately **86.12%**. All of these accuracy measures are fairly high, with over 80% of the songs being correctly predicted as skipped or not skipped. The k-nearest neighbors model was the poorest fitting model. 

Overall, the logistic model had the highest accuracy with a low standard error, suggesting that the logistic model is the best fitting model for this data. Because the data does not have many outliers, and song characteristics are typically related to each other without being entirely separated from one another (e.g., a song with a minor mode and slower tempo is typically related to a lower valence), this model does seem to be a proper choice theoretically.
